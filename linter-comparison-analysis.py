#!/usr/bin/env python3
"""
SLDS Linter Comparison Analysis
...
def parse_linter_output(content: str) -> List[Dict]:
    """Parse linter output and extract violations for no-hardcoded-values-slds2 rule only"""
    violations = []
    ...
    for line in lines:
        ...
        if '⚠' in line and 'slds/no-hardcoded-values-slds2' in line:
            # Extract position information using regex - look for line:col pattern
            ...
            value_match = re.search(r'the ([^s]+?) static value', message_part)
            if not value_match:
                value_match = re.search(r'Consider replacing the ([^s]+?) static', message_part)
            if not value_match:
                value_match = re.search(r'no replacement styling hook for the ([^s]+?) static', message_part)
            ...
            if 'styling hook that has a similar value:' in message_part:
                hooks_part = message_part.split('styling hook that has a similar value:')[1].strip()
                # Extract hook names that start with --slds
                hook_matches = re.findall(r'--slds-[a-z0-9-]+', hooks_part)
                hooks = hook_matches
...
def main():
    ...
    # Read the linter output files
    with open('/Users/ritesh.kumar2/Documents/projects/stylelint-sds/local-uplift-bugs-results.txt', 'r') as f:
        local_uplift_content = f.read()
    
    with open('/Users/ritesh.kumar2/Documents/projects/stylelint-sds/published-uplift-bugs-results.txt', 'r') as f:
        published_uplift_content = f.read()
        
    with open('/Users/ritesh.kumar2/Documents/projects/stylelint-sds/local-hardcoded-values-results.txt', 'r') as f:
        local_hardcoded_content = f.read()
        
    with open('/Users/ritesh.kumar2/Documents/projects/stylelint-sds/published-hardcoded-values-results.txt', 'r') as f:
        published_hardcoded_content = f.read()
    
    # Parse violations
    local_uplift_violations = parse_linter_output(local_uplift_content)
    published_uplift_violations = parse_linter_output(published_uplift_content)
    local_hardcoded_violations = parse_linter_output(local_hardcoded_content)
    published_hardcoded_violations = parse_linter_output(published_hardcoded_content)
    
    # Compare and find differences
    uplift_differences = compare_violations(local_uplift_violations, published_uplift_violations, 'uplift-bugs.css')
    hardcoded_differences = compare_violations(local_hardcoded_violations, published_hardcoded_violations, 'hardcoded-values.css')
    ...
    summary_stats = {
        'uplift-bugs.css': {
            'local_violations': len(local_uplift_violations),
            'published_violations': len(published_uplift_violations),
            'differences_found': len(uplift_differences)
        },
        'hardcoded-values.css': {
            'local_violations': len(local_hardcoded_violations),
            'published_violations': len(published_hardcoded_violations), 
            'differences_found': len(hardcoded_differences)
        }
    }
    
    # Create Excel report
    with pd.ExcelWriter('/Users/ritesh.kumar2/Documents/projects/stylelint-sds/slds-linter-comparison-report.xlsx', engine='openpyxl') as writer:
        ...
        local_uplift_df = pd.DataFrame(local_uplift_violations)
        if not local_uplift_df.empty:
            local_uplift_df.to_excel(writer, sheet_name='Local_Uplift_Bugs', index=False)
            
        published_uplift_df = pd.DataFrame(published_uplift_violations)
        if not published_uplift_df.empty:
            published_uplift_df.to_excel(writer, sheet_name='Published_Uplift_Bugs', index=False)
            
        local_hardcoded_df = pd.DataFrame(local_hardcoded_violations)
        if not local_hardcoded_df.empty:
            local_hardcoded_df.to_excel(writer, sheet_name='Local_Hardcoded_Values', index=False)
            
        published_hardcoded_df = pd.DataFrame(published_hardcoded_violations)
        if not published_hardcoded_df.empty:
            published_hardcoded_df.to_excel(writer, sheet_name='Published_Hardcoded_Values', index=False)
    
    print("=== SLDS Linter Comparison Report ===")
    print(f"\nSummary Statistics:")
    print(f"uplift-bugs.css:")
    print(f"  - Local violations: {summary_stats['uplift-bugs.css']['local_violations']}")
    print(f"  - Published violations: {summary_stats['uplift-bugs.css']['published_violations']}")  
    print(f"  - Differences found: {summary_stats['uplift-bugs.css']['differences_found']}")
    
    print(f"\nhardcoded-values.css:")
    print(f"  - Local violations: {summary_stats['hardcoded-values.css']['local_violations']}")
    print(f"  - Published violations: {summary_stats['hardcoded-values.css']['published_violations']}")
    ...
    print(f"\nExcel report saved to: slds-linter-comparison-report.xlsx")
