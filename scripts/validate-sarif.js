...
// Main execution
function main() {
    const sarifFilePath = path.resolve(process.cwd(), 'slds-linter-report.sarif');
    
    if (!fs.existsSync(sarifFilePath)) {
        console.error('SARIF file not found:', sarifFilePath);
        process.exit(1);
    }
    ...
    const outputPath = path.resolve(process.cwd(), 'sarif-validation-report.html');
